    ...
    <div class="flex flex-col w-full p-6">
        ...
        <div class="flex flex-col mb-4">
            ...
            <textarea 
                id="editor" 
                name="content" 
                class="@error('content') border-red-500 @enderror">{{ old('content', $blog->content) }}</textarea>
            @error('content')
                <span class="text-red-500 text-sm mt-1">{{ $message }}</span>
            @enderror
            <script>
        ...
        <div class="flex flex-col mb-4">
            ...
            <div id="image-preview" class="mb-4">
                @if($blog->image)
                    <img src="{{ asset('storage/' . $blog->image) }}" alt="Blog Image" class="max-w-xs rounded shadow">
                @else
